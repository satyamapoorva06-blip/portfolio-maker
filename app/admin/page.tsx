      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6 text-center font-sans">
        <div className="max-w-md p-8 bg-slate-900 border border-rose-800 rounded-3xl space-y-4">
          ...
          <p className="text-xs text-slate-400">
            Access to /admin is restricted strictly to accounts with
      ...
      <main className="flex-1 p-10 space-y-10 overflow-y-auto">
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-400">
            Real-time statistics across users, portfolios, GitHub integrations,
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            },
            {
              ...
              val: metrics.totalPortfolios,
              sub: `${metrics.publishedPortfolios} published`,
              icon: Folders,
              color: "text-cyan-400",
            },
            {
              label: "Deployments",
              val:
                metrics.vercelDeploymentsCount +
                metrics.netlifyDeploymentsCount,
              sub: "Vercel & Netlify",
              icon: Rocket,
            ...
            {
              label: "Total Views",
              val: metrics.totalPortfolioViews.toLocaleString(),
              sub: "Across public URLs",
              icon: Eye,
            ...
            return (
              <div
                key={idx}
                className="p-6 bg-slate-900 border border-slate-800 rounded-3xl space-y-3 shadow-xl"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl space-y-4">
            <h3 className="font-bold text-base text-white flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-emerald-400" /> Infrastructure
              & Integrations
            </h3>
            <div className="space-y-3 text-xs">
              <div className="flex justify-between items-center p-3 bg-slate-950 rounded-xl">
                ...
                <span className="font-mono text-cyan-400 font-bold">
                  {metrics.githubConnectionsCount}
                </span>
              </div>
              <div className="flex justify-between items-center p-3 bg-slate-950 rounded-xl">
          ...
          <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl space-y-4">
            <h3 className="font-bold text-base text-white flex items-center gap-2">
              <Cpu className="w-5 h-5 text-purple-400" /> Platform Privacy
              Safeguards
            </h3>
            <p className="text-xs text-slate-400 leading-relaxed">
              In compliance with Section 31, no sensitive credentials
              (passwords, OAuth tokens, AI keys, or private environment secrets)
              are exposed in the Admin Panel.
            </p>
