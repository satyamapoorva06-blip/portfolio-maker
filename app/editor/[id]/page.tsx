...
import { getStoredPortfolios, saveStoredPortfolio, saveStoredDeployment, INITIAL_PORTFOLIO } from '@/lib/storage/local-store';
import EditorTabs from '@/components/editor/EditorTabs';
import LivePreviewFrame from '@/components/editor/LivePreviewFrame';
import { ArrowLeft, Save, Rocket, Github, CheckCircle2, Eye, Sparkles, X, ExternalLink, Copy, Loader2, Globe } from 'lucide-react';
...
export default function EditorPage() {
  ...
  const handleOneClickDeploy = async () => {
    ...
    setDeployStatus('1. Creating GitHub repository & pushing Next.js code...');
    ...
    try {
      ...
      saveStoredDeployment({
        ...
        user_id: portfolio.userId || 'usr_demo',
        provider: 'vercel',
    ...
    <div className="h-screen flex flex-col bg-slate-950 text-slate-100 overflow-hidden font-sans relative">
      ...
      {showDeployModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-6">
          <div className="max-w-lg w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 space-y-6 shadow-2xl relative">
            <button
            ...
            <div className="space-y-2 text-center">
              ...
              <p className="text-xs text-slate-400">
                Pushes Next.js portfolio source code to GitHub and publishes your website live on Vercel Edge CDN.
              </p>
            </div>
            ...
            {deployedUrls ? (
              ...
              <div className="space-y-4">
                <div className="p-4 bg-slate-950 border border-slate-800 rounded-2xl space-y-2 text-xs text-slate-300 font-mono">
                  <div className="flex items-center gap-2 text-cyan-400">
                    <Github className="w-4 h-4" /> GitHub: {portfolio.slug}-portfolio
                  </div>
                  <div className="flex items-center gap-2 text-emerald-400">
                    <Globe className="w-4 h-4" /> Vercel: https://{portfolio.slug}.vercel.app
                  </div>
                </div>
                ...
                >
                  ...
                  {deployLoading ? 'Deploying to GitHub & Vercel...' : '🚀 1-Click Deploy Now'}
                </button>
