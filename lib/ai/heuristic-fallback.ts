...
export function parseResumeTextHeuristically(
  ...
  const phoneMatch =
    rawText.match(/(\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/) ||
  ...
  for (const line of lines) {
    if (
      !/resume|curriculum|cv|page|email|phone|github|linkedin|http|contact|profile|summary|objective/i.test(
        line,
      ) &&
      line.length > 2 &&
      line.length < 45 &&
  ...
  if (textLower.includes("full stack") || textLower.includes("fullstack")) {
    ...
    textLower.includes("data analyst")
  ) {
    title = "Data Scientist & Analyst";
  } else if (
    textLower.includes("ai/ml") ||
    textLower.includes("machine learning") ||
    textLower.includes("ai engineer")
  ) {
  ...
  const summaryIndex = lines.findIndex((l) =>
    /^(summary|professional summary|about|about me|objective|career objective|profile)$/i.test(
      l,
    ),
  );
  if (summaryIndex !== -1) {
    ...
    for (
      ...
      i < Math.min(summaryIndex + 7, lines.length);
      i++
    ) {
      if (
        /^(education|skills|projects|experience|work history|certifications|achievements)/i.test(
          lines[i],
        )
      )
        break;
  ...
  projects.slice(0, 2).forEach((p) => {
    if (p.name && p.description)
      highlights.push(`Developed ${p.name}: ${p.description.slice(0, 90)}...`);
  });
  experience.slice(0, 2).forEach((e) => {
    if (e.role && e.company)
      highlights.push(`Worked as ${e.role} at ${e.company}`);
  });
...
function extractSkillsFromResume(lines: string[], rawText: string) {
  ...
  const skillSectionIdx = lines.findIndex((l) =>
    /^(skills|technical skills|skills & tools|technologies|core competencies|skills and frameworks)$/i.test(
      l,
    ),
  );
  ...
  if (skillSectionIdx !== -1) {
    for (let i = skillSectionIdx + 1; i < lines.length; i++) {
      const line = lines[i];
      if (
        /^(projects|experience|work history|education|certifications|achievements|declaration)/i.test(
          line,
        )
      )
        break;
      ...
      parts.forEach((p) => {
        if (
          !/languages|frameworks|tools|databases|libraries|technologies/i.test(
            p,
          )
        ) {
          directSkills.push(p);
  ...
  if (allFound.length > 0) {
    const langs = allFound.filter((s) =>
      /java|python|c\+\+|c#|javascript|typescript|go|rust|html|css|sql|php|ruby|kotlin|swift|dart|r\b/i.test(
        s,
      ),
    );
    const frameworks = allFound.filter((s) =>
      /react|next|node|express|vue|angular|svelte|tailwind|bootstrap|flutter|spring|django|flask|fastapi|pytorch|tensorflow|pandas|numpy/i.test(
        s,
      ),
    );
    const tools = allFound.filter(
      (s) => !langs.includes(s) && !frameworks.includes(s),
    );
    ...
    : [
        {
          ...
          skills: ["Software Development", "Problem Solving", "Git"],
        },
      ];
}
...
function extractProjectsFromText(lines: string[]) {
  const projIdx = lines.findIndex((l) =>
    /^(projects|featured projects|key projects|academic projects|technical projects|personal projects)$/i.test(
      l,
    ),
  );
  if (projIdx === -1) return [];
  ...
  for (let i = projIdx + 1; i < lines.length; i++) {
    const line = lines[i];
    if (
      /^(education|experience|work history|skills|certifications|declaration|achievements)/i.test(
        line,
      )
    )
      break;
...
function extractEducationFromText(lines: string[]) {
  const eduIdx = lines.findIndex((l) =>
    /^(education|academic background|academic qualifications|educational qualifications)$/i.test(
      l,
    ),
  );
  if (eduIdx === -1) return [];
  ...
  for (let i = eduIdx + 1; i < Math.min(eduIdx + 15, lines.length); i++) {
    const line = lines[i];
    if (
      /^(projects|experience|skills|certifications|work history|achievements)/i.test(
        line,
      )
    )
      break;
    ...
    if (
      /bachelor|b\.tech|btech|master|m\.tech|degree|university|college|institute|school|b\.e|b\.sc|m\.sc/i.test(
        line,
      )
    ) {
...
function extractExperienceFromText(lines: string[]) {
  const expIdx = lines.findIndex((l) =>
    /^(experience|work history|employment|professional experience|work experience|internships)$/i.test(
      l,
    ),
  );
  if (expIdx === -1) return [];
  ...
  for (let i = expIdx + 1; i < lines.length; i++) {
    const line = lines[i];
    if (
      /^(projects|education|skills|certifications|declaration|achievements)/i.test(
        line,
      )
    )
      break;
    ...
    if (
      ...
      currentExp = {
        ...
        current:
          !dateMatch || dateMatch.some((d) => /present|current/i.test(d)),
        description: line,
        achievements: [],
        technologies: [],
      };
