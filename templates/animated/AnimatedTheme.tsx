      ...
      <div className="max-w-5xl mx-auto space-y-16 relative z-10">
        ...
        <div className="w-full overflow-hidden bg-gradient-to-r from-purple-900/60 via-cyan-900/60 to-purple-900/60 border border-purple-500/30 rounded-2xl py-2.5 px-4 backdrop-blur-md flex items-center gap-4">
          <span className="flex items-center gap-1.5 text-xs font-mono font-bold text-cyan-400 shrink-0">
            <Sparkles className="w-4 h-4 text-cyan-400 animate-spin" /> ANIMATED
            PORTFOLIO MODE
          </span>
        ...
        {projects && projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
              <Code2 className="w-5 h-5 text-cyan-400" /> Animated Featured
              Projects
            </h2>
        ...
        {skills && skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
              <Layers className="w-5 h-5 text-purple-400" /> Animated Skill
              Matrix
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="p-6 bg-[#0d0f1a]/80 border border-slate-800 rounded-3xl space-y-3 backdrop-blur-xl"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-slate-900 text-slate-200 px-3 py-1.5 rounded-xl border border-slate-800 font-mono transform hover:scale-105 transition"
                      >
                        {s}
                      </span>
                    ))}
