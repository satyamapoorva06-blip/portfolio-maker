    ...
    <div className="min-h-screen bg-[#030712] text-slate-100 font-sans p-6 sm:p-12 relative overflow-x-hidden">
      ...
      <div className="max-w-5xl mx-auto space-y-16 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-violet-900/40 pb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-mono text-violet-400 font-semibold">
                  {personal.title}
                </p>
              </div>
            </div>
            ...
            <span className="px-3.5 py-1.5 bg-violet-950/80 border border-violet-700 text-violet-300 text-xs font-mono rounded-full flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5 text-pink-400 animate-spin" /> 3D
              SPATIAL ORBS
            </span>
          </div>
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    {proj.liveUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-2 text-violet-400 hover:text-white rounded-xl transition"
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {proj.technologies.map((t, i) => (
                      <span
                        key={i}
                        className="text-[10px] bg-violet-950/80 text-violet-300 px-2.5 py-0.5 rounded-lg border border-violet-800 font-mono"
                      >
                        {t}
                      </span>
                    ))}
        ...
        {skills && skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-violet-400" /> SPATIAL & MATRIX
              STACK
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="p-6 bg-[#0b1329]/80 border border-violet-500/30 rounded-3xl space-y-3 backdrop-blur-xl"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-[#131d38] text-slate-200 px-3 py-1 rounded-xl border border-violet-500/20 font-mono transform hover:scale-105 transition"
                      >
                        {s}
                      </span>
                    ))}
