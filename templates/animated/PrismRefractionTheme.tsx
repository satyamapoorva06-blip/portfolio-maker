...
export default function PrismRefractionTheme({
  data,
}: {
  data: PortfolioData;
}) {
        ...
        {projects && projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Layers className="w-5 h-5 text-pink-400" /> PRISM REFRACTION
              WORKS
            </h2>
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    {proj.liveUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-2 text-cyan-300 hover:text-white rounded-xl transition"
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {proj.technologies.map((t, i) => (
                      <span
                        key={i}
                        className="text-[10px] bg-white/10 text-pink-300 px-2.5 py-0.5 rounded-lg border border-white/20 font-mono"
                      >
                        {t}
                      </span>
                    ))}
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="p-6 bg-[#0c0e1e]/80 border border-white/10 rounded-3xl space-y-3 backdrop-blur-xl"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-white/10 text-slate-200 px-3 py-1 rounded-xl border border-white/20 font-mono"
                      >
                        {s}
                      </span>
                    ))}
