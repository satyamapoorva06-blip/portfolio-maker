    ...
    <div className="min-h-screen bg-[#080c14] text-slate-100 font-sans p-6 sm:p-12 relative overflow-x-hidden">
      <div className="max-w-5xl mx-auto space-y-16 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs font-mono text-orange-400 font-bold mt-1">
                  {personal.title}
                </p>
              </div>
            </div>
              ...
              {projects.map((proj) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <h3 className="font-extrabold text-white text-base group-hover:text-orange-400 transition">
                      {proj.name}
                    </h3>
                    {proj.liveUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-2 text-orange-400 hover:text-white rounded-xl transition"
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {proj.technologies.map((t, i) => (
                      <span
                        key={i}
                        className="text-[10px] bg-orange-950/80 text-orange-300 px-2.5 py-0.5 rounded-lg border border-orange-800 font-mono"
                      >
                        {t}
                      </span>
                    ))}
        ...
        {skills && skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Activity className="w-5 h-5 text-blue-400" /> ELASTIC SKILL
              MATRIX
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((cat) => (
                ...
                >
                  <h3 className="text-xs font-bold text-orange-400 font-mono uppercase">
                    {cat.category}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-slate-900 text-slate-200 px-3 py-1.5 rounded-xl border border-slate-700 font-mono transform hover:scale-110 transition"
                      >
                        {s}
                      </span>
                    ))}
