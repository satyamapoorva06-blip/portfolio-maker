...
export default function FullstackSoftwareTheme({ data }: { data: PortfolioData }) {
  ...
  return (
    <div className="min-h-screen bg-[#070a12] text-slate-100 font-sans selection:bg-cyan-500 selection:text-black">
      <header className="border-b border-cyan-950 bg-[#0a101d]/90 backdrop-blur sticky top-0 z-50 px-6 py-4">
      ...
      <footer className="border-t border-cyan-950 py-8 text-center text-xs text-slate-500 font-mono">
        © {new Date().getFullYear()} {p.name}. Full Stack Software Portfolio.
      </footer>
    </div>
  );
}
