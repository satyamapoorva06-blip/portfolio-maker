          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className="text-xs font-mono text-cyan-400 font-bold">
                  {personal.title || "AI & Machine Learning Engineer"}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <span className="px-3.5 py-1.5 bg-purple-950/80 border border-purple-800 text-purple-300 text-xs font-mono rounded-full flex items-center gap-1.5 font-semibold">
                <Bot className="w-4 h-4 text-cyan-400 animate-pulse" /> Neural &
                LLM Systems
              </span>
            </div>
          </div>
        ...
        {projects && projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Network className="w-5 h-5 text-purple-400" /> AI Models &
              Agentic Deployments
            </h2>
                  ...
                  {proj.technologies && proj.technologies.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-2">
                      {proj.technologies.map((t, i) => (
                        <span
                          key={i}
                          className="text-[10px] bg-purple-950/80 text-purple-300 px-2.5 py-0.5 rounded-lg border border-purple-800 font-mono"
                        >
                          {t}
                        </span>
                      ))}
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="p-6 bg-[#0d0f18] border border-slate-800 rounded-3xl space-y-3"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-[#141724] text-slate-200 px-3 py-1 rounded-xl border border-purple-500/20 font-mono"
                      >
                        {s}
                      </span>
                    ))}
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-4">
              {experience.map((exp) => (
                <div
                  key={exp.id}
                  className="p-6 bg-[#0d0f18] border border-purple-500/30 rounded-3xl space-y-3"
                >
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-4">
              {education.map((edu) => (
                <div
                  key={edu.id}
                  className="p-6 bg-[#0d0f18] border border-slate-800 rounded-3xl flex flex-wrap justify-between items-center gap-4"
                >
                  <div>
                    <h3 className="font-bold text-white text-base">
                      {edu.degree}
