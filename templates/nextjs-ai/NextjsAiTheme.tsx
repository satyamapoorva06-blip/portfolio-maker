...
import { Zap, Cpu, Layers, Github, Linkedin } from 'lucide-react';

export default function NextjsAiTheme({ data }: { data: PortfolioData }) {
  const p = data.personal;
  const a = data.about;

  return (
    <div className="min-h-screen bg-[#050910] text-cyan-100 font-sans selection:bg-cyan-500 selection:text-black">
      <header className="border-b border-cyan-900/50 bg-[#08101e]/90 backdrop-blur sticky top-0 z-50 px-6 py-4">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 font-mono text-sm text-cyan-400">
            <Zap className="w-5 h-5 text-cyan-400" />
            <span className="font-bold text-white tracking-wider">{p.name || 'Next.js & AI Engineer'}</span>
          </div>
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-cyan-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-cyan-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Zap className="w-4 h-4 text-cyan-400" /> FRONTEND & AI TOOL INTEGRATOR
              </span>
              <h1 className="text-4xl sm:text-5xl font-black text-white">{p.name}</h1>
              <p className="text-base text-cyan-300 font-mono">{p.title || 'Next.js & AI Developer'}</p>
            </div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-cyan-950 pb-3">
              <Cpu className="w-5 h-5 text-cyan-400" /> Next.js & AI Integration Stack
            </h2>
      ...
      <footer className="border-t border-cyan-950 py-8 text-center text-xs text-slate-500 font-mono">
        © {new Date().getFullYear()} {p.name}. Next.js & AI Tools Portfolio.
      </footer>
    </div>
  );
}
