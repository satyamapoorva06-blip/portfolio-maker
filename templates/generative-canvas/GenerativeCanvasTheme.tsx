...
export default function GenerativeCanvasTheme({
  data,
}: {
  data: PortfolioData;
}) {
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-purple-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-purple-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Palette className="w-4 h-4 text-purple-400" /> GENERATIVE
                CREATIVE AI ARCHITECT
              </span>
              <h1 className="text-4xl sm:text-5xl font-black text-white">
                {p.name}
              </h1>
              <p className="text-base text-purple-300 font-mono">
                {p.title || "Generative AI Developer"}
              </p>
            </div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-purple-950 pb-3">
              <Sparkles className="w-5 h-5 text-purple-400" /> Generative Tools
              & Stack
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#0f0a1e] border border-purple-900/50 rounded-2xl space-y-4"
                >
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-6 bg-[#0f0a1e] border border-purple-900/50 rounded-2xl space-y-4 hover:border-purple-400 transition"
                >
