      ...
      <header className="border-b border-amber-900/50 bg-[#140f09]/90 backdrop-blur sticky top-0 z-50 px-6 py-4">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 font-mono text-sm text-amber-400">
            ...
            <span className="font-bold text-white tracking-wider">{p.name || 'Java Fullstack Architect'}</span>
          </div>
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-amber-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-amber-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Database className="w-4 h-4 text-amber-400" /> JAVA ENTERPRISE & REACT FULLSTACK ARCHITECT
              </span>
              <h1 className="text-4xl sm:text-5xl font-black text-white">{p.name}</h1>
              <p className="text-base text-amber-300 font-mono">{p.title || 'Java Full Stack Engineer'}</p>
            </div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-amber-950 pb-3">
              <Layers className="w-5 h-5 text-amber-400" /> Java, Spring Boot & React Stack
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div key={grp.id} className="p-6 bg-[#1c150c] border border-amber-900/50 rounded-2xl space-y-4">
