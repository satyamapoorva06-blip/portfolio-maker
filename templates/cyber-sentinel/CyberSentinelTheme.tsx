    ...
    <div className="min-h-screen bg-[#060a08] text-emerald-400 font-mono selection:bg-emerald-500 selection:text-black p-6 sm:p-12 relative overflow-x-hidden">
      ...
      <div className="max-w-5xl mx-auto space-y-16 relative z-10">
        ...
        <header className="p-8 bg-[#0b120f] border border-emerald-500/40 rounded-3xl space-y-6 shadow-2xl">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-emerald-900/50 pb-6">
            ...
            <span className="px-3 py-1 bg-emerald-950 text-emerald-300 border border-emerald-800 text-xs rounded-full flex items-center gap-1.5">
              <Shield className="w-3.5 h-3.5 text-emerald-400" /> Cybersecurity
              & Ethical Hacking
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            ...
            {personal.socials.github && (
              ...
              >
                <Github className="w-4 h-4 text-emerald-400" /> Exploits &
                GitHub
              </a>
            )}
          </div>
        ...
        {projects && projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Lock className="w-5 h-5 text-emerald-400" /> Penetration Testing
              & Security Tools
            </h2>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {proj.technologies.map((t, i) => (
                      <span
                        key={i}
                        className="text-[10px] bg-emerald-950/80 text-emerald-300 px-2.5 py-0.5 rounded-lg border border-emerald-800"
                      >
                        {t}
                      </span>
                    ))}
        ...
        {skills && skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Terminal className="w-5 h-5 text-emerald-400" /> Security Arsenal
              & Tools
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="p-6 bg-[#0b120f] border border-emerald-900/60 rounded-3xl space-y-3"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-[#121c17] text-emerald-200 px-3 py-1 rounded-xl border border-emerald-900/50"
                      >
                        {s}
                      </span>
                    ))}
