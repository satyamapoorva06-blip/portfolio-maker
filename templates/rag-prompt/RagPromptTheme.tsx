    ...
    <div className="min-h-screen bg-[#05080c] text-emerald-100 font-mono selection:bg-emerald-500 selection:text-black">
      ...
      <main className="max-w-5xl mx-auto px-6 py-12 space-y-12">
        ...
        <section className="bg-[#09121e] border border-emerald-500/30 rounded-2xl p-8 space-y-6 shadow-2xl relative">
          ...
          <div className="space-y-3 border-l-2 border-emerald-500 pl-4">
            ...
            <p className="text-base text-emerald-400 font-bold">
              {p.title || "GenAI & RAG Prompt Architect"}
            </p>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-4">
            <div className="text-xs text-emerald-400">
              $ list_vector_embeddings --categories
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-5 bg-[#09121e] border border-emerald-900/50 rounded-xl space-y-3"
                >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-0.5 bg-emerald-950/80 border border-emerald-800/80 text-emerald-300 text-xs rounded font-mono"
                      >
                        {sk}
                      </span>
                    ))}
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="space-y-4">
            <div className="text-xs text-emerald-400">
              $ fetch_rag_projects --list
            </div>
            <div className="space-y-4">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-6 bg-[#09121e] border border-emerald-900/50 rounded-2xl space-y-3"
                >
                  ...
                  {proj.technologies && proj.technologies.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {proj.technologies.map((t, idx) => (
                        <span
                          key={idx}
                          className="text-[10px] bg-black text-emerald-400 px-2 py-0.5 rounded border border-emerald-900"
                        >
                          {t}
                        </span>
                      ))}
