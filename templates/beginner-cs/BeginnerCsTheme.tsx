    ...
    <div className="min-h-screen bg-[#070e1b] text-slate-200 font-sans selection:bg-blue-500 selection:text-white">
      ...
      <main className="max-w-5xl mx-auto px-6 py-12 space-y-12">
          ...
          <div className="flex flex-wrap justify-between items-start gap-4">
            <div className="space-y-2">
              <span className="text-xs font-mono text-blue-400 uppercase tracking-widest font-semibold flex items-center gap-1.5">
                <BookOpen className="w-4 h-4 text-blue-400" /> COMPUTER SCIENCE
                & ENGINEERING SCHOLAR
              </span>
            ...
            <div className="flex gap-3">
              {p.socials?.github && (
                <a
                  ...
                  className="p-2.5 bg-slate-900 border border-slate-800 rounded-xl text-slate-300 hover:text-blue-400 transition"
                >
        ...
        {data.education && data.education.length > 0 && (
          <section className="space-y-4">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-blue-900/50 pb-3">
              <GraduationCap className="w-5 h-5 text-blue-400" /> Academic
              Degree & Institution
            </h2>
            <div className="grid grid-cols-1 gap-4">
              {data.education.map((edu) => (
                <div
                  key={edu.id}
                  className="p-6 bg-[#0c1a30] border border-blue-900/50 rounded-2xl flex flex-wrap justify-between items-center gap-4"
                >
                  <div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-4">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-blue-900/50 pb-3">
              <Code2 className="w-5 h-5 text-blue-400" /> Programming
              Fundamentals & Technical Stack
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#0c1a30] border border-blue-900/50 rounded-2xl space-y-3"
                >
                  <h3 className="text-xs font-bold text-blue-300 font-mono uppercase">
                    {grp.category}
                  </h3>
                  <div className="flex flex-wrap gap-1.5">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-2.5 py-1 bg-blue-950 text-blue-200 text-xs font-mono rounded-lg border border-blue-800"
                      >
                        {sk}
                      </span>
                    ))}
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="space-y-4">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-blue-900/50 pb-3">
              <Cpu className="w-5 h-5 text-blue-400" /> Coursework & Engineering
              Projects
            </h2>
