...
export default function ReactiveFullstackTheme({
  data,
}: {
  data: PortfolioData;
}) {
    ...
    <div className="min-h-screen bg-[#070e17] text-slate-100 font-sans selection:bg-cyan-500 selection:text-black">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-12">
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-cyan-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-cyan-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Activity className="w-4 h-4 text-cyan-400" /> FULLSTACK
                REACTIVE SYSTEM ARCHITECT
              </span>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-cyan-950 pb-3">
              <Server className="w-5 h-5 text-cyan-400" /> Reactive Tech Stack &
              Infrastructure
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#0b1b2e] border border-cyan-900/50 rounded-2xl space-y-4"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-cyan-950/80 border border-cyan-800/80 text-cyan-200 text-xs font-mono rounded-lg"
                      >
                        {sk}
                      </span>
                    ))}
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-cyan-950 pb-3">
              <Layers className="w-5 h-5 text-cyan-400" /> Web Applications &
              Fullstack Services
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-6 bg-[#0b1b2e] border border-cyan-900/50 rounded-2xl space-y-4 hover:border-cyan-400 transition"
                >
                  ...
                  {proj.technologies && proj.technologies.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-2">
                      {proj.technologies.map((t, idx) => (
                        <span
                          key={idx}
                          className="text-[10px] bg-black text-cyan-300 px-2.5 py-1 rounded font-mono border border-cyan-950"
                        >
                          {t}
                        </span>
                      ))}
