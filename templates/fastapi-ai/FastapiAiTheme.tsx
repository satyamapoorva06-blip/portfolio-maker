    ...
    <div className="min-h-screen bg-[#050b07] text-emerald-100 font-mono selection:bg-emerald-500 selection:text-black">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-12">
        <section className="p-8 bg-[#0b1b11] border border-emerald-500/30 rounded-3xl space-y-6 shadow-2xl">
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-emerald-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs text-emerald-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Zap className="w-4 h-4 text-emerald-400" /> FASTAPI ASYNC
                BACKEND & AI MODEL ARCHITECT
              </span>
              <h1 className="text-4xl sm:text-5xl font-black text-white">
                {p.name}
              </h1>
              <p className="text-base text-emerald-300">
                {p.title || "Python FastAPI AI Developer"}
              </p>
            </div>
            <div className="flex gap-3">
              {p.socials?.github && (
                <a
                  ...
                  className="p-2.5 bg-slate-900 border border-slate-800 rounded-xl text-slate-300 hover:text-emerald-400 transition"
                >
          ...
          <p className="text-slate-300 text-sm leading-relaxed max-w-3xl font-light font-sans">
            {a.summary}
          </p>
        </section>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-emerald-950 pb-3">
              <Cpu className="w-5 h-5 text-emerald-400" /> Python, FastAPI & AI
              Inference Stack
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#0b1b11] border border-emerald-900/50 rounded-2xl space-y-4"
                >
                  <h3 className="text-sm font-bold text-emerald-300 uppercase">
                    {grp.category}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-emerald-950 text-emerald-200 text-xs rounded-lg border border-emerald-800"
                      >
                        {sk}
                      </span>
                    ))}
