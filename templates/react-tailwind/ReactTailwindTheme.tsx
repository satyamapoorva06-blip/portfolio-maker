    ...
    <div className="min-h-screen bg-[#06101e] text-slate-100 font-sans selection:bg-cyan-400 selection:text-black">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-12">
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-cyan-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-cyan-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Palette className="w-4 h-4 text-cyan-400" /> FRONTEND &
                COMPONENT ARCHITECT
              </span>
              ...
              <p className="text-base text-cyan-300 font-mono">
                {p.title || "React & Tailwind UI Engineer"}
              </p>
            </div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-cyan-950 pb-3">
              <Code className="w-5 h-5 text-cyan-400" /> Frontend Libraries &
              Design System Stack
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#0c1f3b] border border-cyan-900/50 rounded-2xl space-y-4"
                >
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-cyan-950 pb-3">
              <Layout className="w-5 h-5 text-cyan-400" /> React Web
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-6 bg-[#0c1f3b] border border-cyan-900/50 rounded-2xl space-y-4 hover:border-cyan-400 transition"
                >
