      ...
      <main className="max-w-4xl mx-auto px-6 py-16 space-y-12">
        <section className="space-y-4 border-b border-slate-800 pb-8">
          ...
          <p className="text-sm font-mono text-slate-400">
            {p.title || "Minimalist CS Developer"}
          </p>
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-4 bg-slate-900/50 border border-slate-800/80 rounded-xl space-y-2"
                >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="text-xs bg-slate-950 text-slate-300 px-2 py-0.5 rounded border border-slate-800 font-mono"
                      >
                        {sk}
                      </span>
                    ))}
