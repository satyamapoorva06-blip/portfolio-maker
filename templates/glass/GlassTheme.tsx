        ...
        <section className="backdrop-blur-2xl bg-white/[0.04] border border-white/10 rounded-3xl p-8 sm:p-12 space-y-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-400/20 text-xs font-bold text-cyan-300">
              <Sparkles className="w-3.5 h-3.5" /> AMBIENT FROSTED GLASS
              PORTFOLIO
            </div>
          ...
          {about.highlights && about.highlights.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 max-w-3xl">
              {about.highlights.map((h, idx) => (
                <div
                  key={idx}
                  className="p-3.5 backdrop-blur-xl bg-white/[0.03] border border-white/10 rounded-2xl text-xs text-slate-200 flex items-start gap-2.5"
                >
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2 pt-3 border-t border-white/10">
                    {proj.technologies.map((t, idx) => (
                      <span
                        key={idx}
                        className="text-xs bg-cyan-500/10 text-cyan-300 font-mono px-3 py-1 rounded-lg border border-cyan-500/20"
                      >
                        {t}
                      </span>
                    ))}
        ...
        {sectionVisibility.skills && skills.length > 0 && (
          <section className="space-y-8">
            <div className="border-b border-white/10 pb-4">
              <h2 className="text-2xl font-extrabold text-white flex items-center gap-3">
                <Sparkles className="w-6 h-6 text-purple-400" /> Technical
                Ecosystem
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="backdrop-blur-xl bg-white/[0.03] border border-white/10 rounded-3xl p-6 space-y-4"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, idx) => (
                      <span
                        key={idx}
                        className="text-xs bg-white/5 hover:bg-white/10 text-slate-200 font-mono px-3 py-1.5 rounded-xl border border-white/10 transition"
                      >
                        {s}
                      </span>
                    ))}
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {education.map((edu) => (
                <div
                  key={edu.id}
                  className="backdrop-blur-xl bg-white/[0.03] border border-white/10 rounded-3xl p-6 space-y-2"
                >
                  <div className="flex justify-between items-start">
                    <div>
