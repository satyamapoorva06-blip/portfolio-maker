          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-blue-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs text-blue-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Binary className="w-4 h-4 text-blue-400" /> ALGORITHMS & DATA
                STRUCTURE ARCHITECT
              </span>
              ...
              <p className="text-base text-blue-300">
                {p.title || "CS Algorithm Systems Developer"}
              </p>
            </div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-blue-950 pb-3">
              <Cpu className="w-5 h-5 text-blue-400" /> Algorithmic Stack &
              Complexity Analysis
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#0c162d] border border-blue-900/50 rounded-2xl space-y-4"
                >
                  <h3 className="text-sm font-bold text-blue-300 uppercase">
                    {grp.category}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-blue-950 text-blue-200 text-xs rounded-lg border border-blue-800"
                      >
                        {sk}
                      </span>
                    ))}
