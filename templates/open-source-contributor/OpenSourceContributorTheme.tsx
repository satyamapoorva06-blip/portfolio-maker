...
export default function OpenSourceContributorTheme({
  data,
}: {
  data: PortfolioData;
}) {
    ...
    <div className="min-h-screen bg-[#0d1117] text-[#c9d1d9] font-mono p-6 sm:p-12 relative overflow-x-hidden">
      <div className="max-w-5xl mx-auto space-y-12 relative z-10">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-[#8b949e]">
                  @{data.slug || "developer"} • {personal.title}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-2 pt-2">
            <div className="flex justify-between items-center text-[10px] text-[#8b949e]">
              <span className="flex items-center gap-1.5 font-bold">
                <GitCommit className="w-3.5 h-3.5 text-emerald-400" /> 1,420
          ...
          <section className="space-y-6">
            <div className="flex justify-between items-center border-b border-[#30363d] pb-3">
              <h2 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
                <GitPullRequest className="w-4 h-4 text-emerald-400" /> Pinned
                Repositories & Open Source PRs
              </h2>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {proj.technologies.map((t, i) => (
                      <span
                        key={i}
                        className="text-[10px] bg-[#21262d] text-[#c9d1d9] px-2.5 py-0.5 rounded-full border border-[#30363d]"
                      >
                        ● {t}
                      </span>
                    ))}
        ...
        {skills && skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2 border-b border-[#30363d] pb-3">
              <Terminal className="w-4 h-4 text-emerald-400" /> Developer
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="p-6 bg-[#161b22] border border-[#30363d] rounded-2xl space-y-3"
                >
                  <h3 className="text-xs font-bold text-[#58a6ff] uppercase">
                    {cat.category}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-[#21262d] text-[#c9d1d9] px-3 py-1 rounded-xl border border-[#30363d]"
                      >
                        {s}
                      </span>
                    ))}
        ...
        <footer className="pt-8 border-t border-[#30363d] text-center text-xs text-[#8b949e]">
          ...
          Open Source Contributor Engine.
        </footer>
      </div>
    </div>
  );
