...
export default function AcademicResearchTheme({
  data,
}: {
  data: PortfolioData;
}) {
      ...
      <div className="max-w-5xl mx-auto space-y-16 relative z-10">
        ...
        <header className="border-b border-slate-800 pb-8 space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="space-y-2">
              <span className="text-xs font-mono uppercase tracking-widest text-indigo-400 font-bold flex items-center gap-2">
                <Landmark className="w-4 h-4" /> ACADEMIC & COMPUTER SCIENCE
                RESEARCH PORTFOLIO
              </span>
          ...
          <div className="p-6 bg-[#101524] border border-indigo-900/40 rounded-2xl space-y-3 font-sans">
            <h2 className="text-xs font-mono uppercase text-indigo-400 font-bold">
          ...
          <section className="space-y-6 font-sans">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {education.map((edu) => (
                <div
                  key={edu.id}
                  className="p-6 bg-[#101524] border border-slate-800 rounded-2xl space-y-2"
                >
        ...
        {projects && projects.length > 0 && (
          <section className="space-y-6 font-sans">
            <h2 className="text-lg font-bold text-white uppercase tracking-wider font-mono flex items-center gap-2 border-b border-slate-800 pb-2">
              <FileText className="w-5 h-5 text-indigo-400" /> Research
              Publications & Technical Projects
            </h2>
            ...
            <div className="space-y-6">
              {projects.map((proj, idx) => (
                <div
                  key={proj.id}
                  className="p-6 bg-[#101524] border border-slate-800 hover:border-indigo-500/50 rounded-2xl space-y-3 transition"
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {proj.technologies.map((t, i) => (
                      <span
                        key={i}
                        className="text-[10px] bg-slate-900 text-indigo-300 px-2.5 py-1 rounded-lg border border-slate-800 font-mono"
                      >
                        {t}
                      </span>
                    ))}
        ...
        {skills && skills.length > 0 && (
          <section className="space-y-6 font-sans">
            <h2 className="text-lg font-bold text-white uppercase tracking-wider font-mono flex items-center gap-2 border-b border-slate-800 pb-2">
              <BookOpen className="w-5 h-5 text-indigo-400" /> Technical Domain
              Knowledge
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="p-6 bg-[#101524] border border-slate-800 rounded-2xl space-y-3"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-slate-900 text-slate-200 px-3 py-1 rounded-xl border border-slate-800 font-mono"
                      >
                        {s}
                      </span>
                    ))}
