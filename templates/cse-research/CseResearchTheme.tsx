          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-indigo-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-indigo-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <GraduationCap className="w-4 h-4 text-indigo-400" /> COMPUTER
                SCIENCE & ENGINEERING RESEARCHER
              </span>
              <h1 className="text-4xl sm:text-5xl font-black text-white">
                {p.name}
              </h1>
              <p className="text-base text-indigo-300 font-mono">
                {p.title || "CSE Research Scholar"}
              </p>
            </div>
        ...
        {data.education && data.education.length > 0 && (
          <section className="space-y-4">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-indigo-950 pb-3">
              <GraduationCap className="w-5 h-5 text-indigo-400" /> Academic
              Degree & Research University
            </h2>
            <div className="grid grid-cols-1 gap-4">
              {data.education.map((edu) => (
                <div
                  key={edu.id}
                  className="p-6 bg-[#0d1830] border border-indigo-900/50 rounded-2xl flex flex-wrap justify-between items-center gap-4"
                >
                  <div>
