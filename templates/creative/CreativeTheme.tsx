        ...
        <div className="flex items-center gap-4 font-bold text-xs">
          {personal.socials.github && (
            <a
              ...
              className="hover:text-purple-600 transition hidden sm:inline"
            >
              GitHub
            </a>
          )}
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-20">
        ...
        <section className="p-8 sm:p-12 rounded-3xl border-3 border-slate-950 bg-gradient-to-br from-purple-100 via-yellow-50 to-pink-100 shadow-[10px_10px_0px_0px_rgba(15,23,42,1)] relative overflow-hidden space-y-8">
          <div className="space-y-6 max-w-4xl">
            ...
            <p className="text-base text-slate-800 leading-relaxed max-w-2xl font-medium">
              {about.summary}
            </p>
            ...
            {about.highlights && about.highlights.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                {about.highlights.map((h, idx) => (
                  <div
                    key={idx}
                    className="p-3.5 bg-white border-2 border-slate-950 rounded-xl text-xs font-bold text-slate-900 flex items-start gap-2.5 shadow-[3px_3px_0px_0px_rgba(15,23,42,1)]"
                  >
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2 pt-3 border-t-2 border-slate-950/20">
                    {proj.technologies.map((tech, i) => (
                      <span
                        key={i}
                        className="text-xs font-black bg-white text-slate-950 px-3 py-1 rounded-lg border-2 border-slate-950 shadow-[2px_2px_0px_0px_rgba(15,23,42,1)]"
                      >
                        {tech}
                      </span>
                    ))}
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="p-6 bg-white rounded-2xl border-3 border-slate-950 space-y-4 shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]"
                >
                  <h3 className="font-black text-slate-950 text-base uppercase tracking-wide border-b-2 border-slate-200 pb-2">
                    {cat.category}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs font-bold bg-slate-100 text-slate-900 px-3 py-1.5 rounded-lg border-2 border-slate-950"
                      >
                        {s}
                      </span>
                    ))}
        ...
        {sectionVisibility.education && education.length > 0 && (
          <section className="space-y-8">
            <div className="border-b-4 border-slate-950 pb-4">
              <h2 className="text-3xl sm:text-4xl font-black tracking-tight text-slate-950 uppercase flex items-center gap-3">
                <GraduationCap className="w-8 h-8 text-amber-500" /> Education &
                Academics
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {education.map((edu) => (
                <div
                  key={edu.id}
                  className="p-6 bg-white rounded-2xl border-3 border-slate-950 space-y-3 shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]"
                >
                  <div className="flex justify-between items-start">
                    <div>
