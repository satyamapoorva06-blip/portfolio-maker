  ...
  return (
    <div className="min-h-screen bg-[#0d0408] text-rose-100 font-mono selection:bg-rose-500 selection:text-black">
      <div className="bg-rose-950 border-b border-rose-800 text-rose-300 text-xs font-bold py-2 px-6 text-center tracking-widest uppercase flex items-center justify-center gap-2">
        ...
        CYBERPUNK MERN STACK & FULLSTACK ARCHITECT
      </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-12">
        <section className="p-8 bg-[#18070e] border-2 border-rose-500/50 rounded-3xl space-y-6 shadow-2xl relative">
            ...
            <div className="flex gap-3">
              {p.socials?.github && (
                <a
                  ...
                  className="p-2.5 bg-black border border-rose-900 rounded-xl text-rose-400 hover:bg-rose-500 hover:text-black transition"
                >
          ...
          <p className="text-rose-400 font-bold text-base">
            {p.title || "Cyberpunk MERN Engineer"}
          </p>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#18070e] border border-rose-900/60 rounded-2xl space-y-4"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-black text-rose-300 text-xs font-bold rounded border border-rose-800"
                      >
                        {sk}
                      </span>
                    ))}
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-rose-900 pb-3">
              <Terminal className="w-5 h-5 text-rose-400" /> Cyber Matrix
              Projects
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-6 bg-[#18070e] border border-rose-900/60 rounded-2xl space-y-4 hover:border-rose-400 transition"
                >
