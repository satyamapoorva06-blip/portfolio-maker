...
export default function SpringbootEnterpriseTheme({
  data,
}: {
  data: PortfolioData;
}) {
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-amber-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-amber-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-amber-400" /> ENTERPRISE
              ...
              <p className="text-base text-amber-300 font-mono">
                {p.title || "Java Backend Systems Engineer"}
              </p>
            </div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-amber-950 pb-3">
              <Database className="w-5 h-5 text-amber-400" /> Enterprise Java
              Stack & Microservices
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#101d16] border border-amber-900/50 rounded-2xl space-y-4"
                >
