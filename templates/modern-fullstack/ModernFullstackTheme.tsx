...
export default function ModernFullstackTheme({
  data,
}: {
  data: PortfolioData;
}) {
    ...
    <div className="min-h-screen bg-[#070d18] text-slate-100 font-sans selection:bg-blue-500 selection:text-white">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-12">
        <section className="p-8 bg-[#0c1a32] border border-blue-500/30 rounded-3xl space-y-6 shadow-2xl relative">
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-blue-900/50 pb-6">
            <div className="space-y-2">
              ...
              <p className="text-base text-blue-300 font-mono">
                {p.title || "Modern Full Stack Engineer"}
              </p>
            </div>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#0c1a32] border border-blue-900/50 rounded-2xl space-y-4"
                >
                  <h3 className="text-sm font-bold text-blue-300 font-mono uppercase">
                    {grp.category}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-blue-950 text-blue-200 text-xs font-mono rounded-lg border border-blue-800"
                      >
                        {sk}
                      </span>
                    ))}
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-blue-950 pb-3">
              <Layers className="w-5 h-5 text-blue-400" /> Fullstack
              Applications
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-6 bg-[#0c1a32] border border-blue-900/50 rounded-2xl space-y-4 hover:border-blue-400 transition"
                >
