          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-8 bg-zinc-900/60 border border-zinc-800/80 rounded-3xl space-y-4 hover:border-rose-500/50 transition group"
                >
                  <div className="flex justify-between items-start">
                    <h3 className="font-bold text-white text-2xl group-hover:text-rose-400 transition">
                      {proj.name}
                    </h3>
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-zinc-900/60 border border-zinc-800/80 rounded-2xl space-y-4"
                >
                  <h3 className="text-xs font-bold text-rose-300 font-mono uppercase">
                    {grp.category}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-zinc-950 border border-zinc-800 text-zinc-300 text-xs font-mono rounded-lg"
                      >
                        {sk}
                      </span>
                    ))}
