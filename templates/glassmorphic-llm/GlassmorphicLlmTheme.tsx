...
export default function GlassmorphicLlmTheme({
  data,
}: {
  data: PortfolioData;
}) {
    ...
    <div className="min-h-screen bg-[#050711] text-indigo-100 font-sans selection:bg-blue-500 selection:text-white">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-12">
            ...
            <div className="flex gap-3">
              {p.socials?.github && (
                <a
                  ...
                  className="p-2.5 bg-slate-900/80 border border-slate-800 rounded-xl text-slate-300 hover:text-indigo-400 transition"
                >
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#0c132f]/80 backdrop-blur-xl border border-indigo-900/50 rounded-2xl space-y-4"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-indigo-950/80 text-indigo-200 text-xs font-mono rounded-lg border border-indigo-800"
                      >
                        {sk}
                      </span>
                    ))}
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-indigo-950 pb-3">
              <Layers className="w-5 h-5 text-indigo-400" /> Glassmorphic LLM
              Projects
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-6 bg-[#0c132f]/80 backdrop-blur-xl border border-indigo-900/50 rounded-2xl space-y-4 hover:border-indigo-400 transition"
                >
      ...
      <footer className="border-t border-indigo-950 py-8 text-center text-xs text-slate-500 font-mono">
        © {new Date().getFullYear()} {p.name}. Glassmorphic LLM Studio
        Portfolio.
      </footer>
    </div>
  );
