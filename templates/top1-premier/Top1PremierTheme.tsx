...
export default function Top1PremierTheme({ data }: { data: PortfolioData }) {
  ...
  >([
    {
      type: "output",
      text: `Welcome to ${personal.name?.toLowerCase().replace(/\s+/g, "") || "user"}@portfolio ~ % Type "help" to list available commands.`,
    },
  ]);
        ...
        const projectsText =
          projects && projects.length > 0
            ? projects
                .map(
                  (p, i) =>
                    `${i + 1}. ${p.name} -> ${p.description} (Tech: ${p.technologies.join(", ")})`,
                )
                .join("\n")
        ...
        const expText =
          experience && experience.length > 0
            ? experience
                .map(
                  (e, i) =>
                    `${i + 1}. ${e.role} @ ${e.company} (${e.startDate} - ${e.current ? "Present" : e.endDate})`,
                )
                .join("\n")
      ...
      case "education":
        const eduText =
          education && education.length > 0
            ? education
                .map((ed) => `${ed.degree} in ${ed.field} @ ${ed.institution}`)
                .join("\n")
  ...
  const processSteps = [
    {
      ...
      desc: "Deconstruct requirements, identify edge cases, and define clear input/output specifications.",
      icon: Search,
    },
    {
      num: "02",
      title: "Algorithm & Architecture",
      desc: "Select optimal data structures, design clean modular architecture, and plan system flow.",
      icon: Cpu,
    },
    {
      num: "03",
      title: "Clean Implementation",
      desc: "Develop maintainable, production-ready code using React, Node.js, Python, and SQL.",
      icon: Code2,
    },
    {
      num: "04",
      title: "Testing & Optimization",
      desc: "Perform thorough debugging, test API contracts, and optimize memory and execution runtime.",
      icon: ShieldCheck,
    },
    {
      num: "05",
      title: "Deployment & Launch",
      desc: "Build CI/CD pipelines, deploy to cloud hosts (Vercel/Netlify), and verify live stability.",
      icon: Rocket,
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#ffffff] font-sans selection:bg-[#e50914] selection:text-white relative overflow-x-hidden">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-24 relative z-10">
        ...
        <section
          id="top"
          className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-12 items-center pt-6"
        >
          <div className="flex flex-col items-start space-y-6">
            ...
            <h1 className="font-display text-4xl sm:text-6xl lg:text-7xl font-bold leading-[1.05] tracking-tight text-white">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#ff3b47] via-[#e50914] to-[#990000]">
                precision
              </span>{" "}
              & AI intelligence
            </h1>
            ...
            <div className="mt-8 flex items-center gap-6 pt-6 border-t border-white/10 w-full max-w-lg">
              ...
              <div className="ml-auto flex items-center gap-1.5 text-xs text-white/50">
                ...
                {personal.location || "Mathura, India"}
              </div>
            </div>
          </div>
              ...
              <div className="glass-card relative overflow-hidden rounded-3xl p-3 border border-white/15 shadow-2xl">
                <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl bg-zinc-900">
                  <img
                    src={
                      personal.avatarUrl ||
                      "https://avatars.githubusercontent.com/u/231909239?v=4"
                    }
                    alt={personal.name}
                  ...
                  <div className="absolute bottom-4 left-4 right-4 rounded-xl border border-white/10 bg-[#050505]/80 p-4 backdrop-blur-md flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-white/60">
                        {personal.title || "B.Tech AI & ML Undergraduate"}
                      </p>
                    </div>
              ...
              <div className="space-y-3">
                {processSteps.map((step, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="group flex items-start gap-5 rounded-2xl p-3 transition-colors hover:bg-white/[0.02]"
                    >
                      ...
                      <div className="pt-1.5">
                        <div className="flex items-center gap-3">
                          <span className="font-display text-sm font-bold text-[#e50914]">
                            {step.num}
                          </span>
            ...
            <div>
              <div className="relative flex h-full flex-col justify-between overflow-hidden rounded-3xl bg-gradient-to-br from-[#990000] via-[#70040a] to-[#400206] p-8 shadow-2xl shadow-[#e50914]/20">
                ...
                <p className="mt-4 font-display text-2xl font-bold leading-snug text-white sm:text-3xl">
                  “Clean code and optimal algorithms aren't just details — they
                  are the foundation of great software.”
                </p>
                <div className="mt-8">
                  ...
                  <span className="mt-3 inline-block text-2xl text-white/80">
                    ✦
                  </span>
                </div>
              </div>
              ...
              {projects.map((proj) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <h3 className="font-display font-bold text-white text-xl group-hover:text-[#e50914] transition">
                      {proj.name}
                    </h3>
                    {proj.liveUrl && (
                      <a
                  ...
                  {proj.technologies && proj.technologies.length > 0 && (
                    <div className="flex flex-wrap gap-2 pt-2 border-t border-white/5">
                      {proj.technologies.map((tech, i) => (
                        <span
                          key={i}
                          className="text-[10px] font-mono font-bold bg-white/5 text-white/80 px-3 py-1 rounded-lg border border-white/10"
                        >
                          {tech}
                        </span>
                      ))}
            ...
            <div className="flex items-center justify-between border-b border-white/10 bg-white/[0.03] px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-3 font-mono text-xs text-white/50 flex items-center gap-1.5">
                  <TerminalIcon className="h-3.5 w-3.5 text-[#e50914]" />{" "}
                  {personal.name?.toLowerCase().replace(/\s+/g, "") || "user"}
                  @portfolio — zsh
                </span>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="glass-card p-7 space-y-4">
                ...
                <p className="text-sm leading-relaxed text-white/60">
                  Continuously deepening knowledge in Data Structures,
                  Algorithms, system design, and AI integrations. I focus on
                  writing performant code and delivering full-stack solutions.
                </p>
              </div>
            </div>
          </section>
        ...
        <footer className="pt-12 border-t border-white/10 text-center text-xs text-white/50 font-mono space-y-3">
          ...
          <p>
            © {new Date().getFullYear()} {personal.name}. Top 1 Premier Flagship
            Portfolio.
          </p>
        </footer>
      </main>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="w-full max-w-lg bg-[#0d0d12] border border-[#e50914]/50 rounded-3xl shadow-2xl overflow-hidden flex flex-col h-[520px]">
            ...
            <form
              ...
              className="p-3 bg-[#14141d] border-t border-white/10 flex gap-2"
            >
              <input
                type="text"
                value={aiInput}
