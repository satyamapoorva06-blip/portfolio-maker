...
import { Server, Activity, Cpu, Layers, Github, Linkedin } from 'lucide-react';

export default function CreativeDevopsTheme({ data }: { data: PortfolioData }) {
  const p = data.personal;
  const a = data.about;

  return (
    <div className="min-h-screen bg-[#060e1a] text-cyan-100 font-sans selection:bg-blue-500 selection:text-white">
      <header className="border-b border-cyan-900/50 bg-[#09172a]/90 backdrop-blur sticky top-0 z-50 px-6 py-4">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 font-mono text-sm text-cyan-400">
            <Server className="w-5 h-5 text-cyan-400" />
            <span className="font-bold text-white tracking-wider">{p.name || 'DevOps Systems'}</span>
          </div>
          <span className="px-3 py-1 bg-cyan-950 text-cyan-300 border border-cyan-800 rounded-full text-xs font-mono font-bold">
            CREATIVE SYSTEMS DEVOPS
          </span>
        </div>
      </header>

      <main className="max-w-6xl mx-auto px-6 py-12 space-y-12">
        <section className="p-8 bg-[#0c1d34] border border-cyan-500/30 rounded-3xl space-y-6 shadow-2xl">
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-cyan-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-cyan-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Activity className="w-4 h-4 text-cyan-400" /> SYSTEMS & DEVOPS AUTOMATION ENGINEER
              </span>
              <h1 className="text-4xl sm:text-5xl font-black text-white">{p.name}</h1>
              <p className="text-base text-cyan-300 font-mono">{p.title || 'DevOps Systems Developer'}</p>
            </div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-cyan-950 pb-3">
              <Cpu className="w-5 h-5 text-cyan-400" /> Systems Stack & CI/CD Infrastructure
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div key={grp.id} className="p-6 bg-[#0c1d34] border border-cyan-900/50 rounded-2xl space-y-4">
      ...
      <footer className="border-t border-cyan-950 py-8 text-center text-xs text-slate-500 font-mono">
        © {new Date().getFullYear()} {p.name}. Creative Systems DevOps Portfolio.
      </footer>
    </div>
  );
}
