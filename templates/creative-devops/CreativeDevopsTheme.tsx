  ...
  return (
    <div className="min-h-screen bg-[#060e1a] text-cyan-100 font-sans selection:bg-blue-500 selection:text-white">
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-cyan-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-cyan-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Activity className="w-4 h-4 text-cyan-400" /> SYSTEMS & DEVOPS
                AUTOMATION ENGINEER
              </span>
              ...
              <p className="text-base text-cyan-300 font-mono">
                {p.title || "DevOps Systems Developer"}
              </p>
            </div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-cyan-950 pb-3">
              <Cpu className="w-5 h-5 text-cyan-400" /> Systems Stack & CI/CD
              Infrastructure
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#0c1d34] border border-cyan-900/50 rounded-2xl space-y-4"
                >
      ...
      <footer className="border-t border-cyan-950 py-8 text-center text-xs text-slate-500 font-mono">
        © {new Date().getFullYear()} {p.name}. Creative Systems DevOps
        Portfolio.
      </footer>
    </div>
  );
