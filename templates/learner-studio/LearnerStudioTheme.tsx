      ...
      <main className="max-w-4xl mx-auto px-6 py-16 space-y-12">
        <section className="space-y-4 border-b border-zinc-800 pb-8">
          ...
          <p className="text-sm font-mono text-zinc-400">
            {p.title || "Student Learner Developer"}
          </p>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-4">
            <h2 className="text-xs font-mono text-zinc-500 uppercase font-bold tracking-widest">
              LEARNING STACK
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl space-y-2"
                >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="text-xs bg-zinc-950 text-zinc-300 px-2 py-0.5 rounded border border-zinc-800 font-mono"
                      >
                        {sk}
                      </span>
                    ))}
