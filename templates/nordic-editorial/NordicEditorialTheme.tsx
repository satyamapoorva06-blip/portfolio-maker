...
export default function NordicEditorialTheme({
  data,
}: {
  data: PortfolioData;
}) {
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-white selection:text-black relative">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 p-6 sm:p-10 flex justify-between items-center backdrop-blur-md bg-black/40 border-b border-zinc-900">
        <div>
          ...
          <span className="text-[11px] text-zinc-500 tracking-wider uppercase font-mono block">
            {personal.title}
          </span>
        </div>
      ...
      <main className="pt-32 px-6 sm:px-16 max-w-6xl mx-auto space-y-32 pb-24">
        ...
        <section className="min-h-[60vh] flex flex-col justify-center space-y-8 border-b border-zinc-900 pb-20">
          ...
          <h2 className="text-4xl sm:text-7xl font-serif leading-tight font-normal text-zinc-100 max-w-4xl">
            {about.summary ||
              `${personal.name} is a ${personal.title} crafting digital systems and interactive web experiences.`}
          </h2>
          ...
          <section className="space-y-16">
            ...
            <div className="space-y-24">
              {projects.map((proj, idx) => (
                <div
                  key={proj.id}
                  className="group border-b border-zinc-900 pb-16 space-y-6"
                >
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {proj.technologies.map((t, i) => (
                      <span
                        key={i}
                        className="text-[11px] font-mono text-zinc-400 border border-zinc-800 px-3 py-1 rounded-full"
                      >
                        {t}
                      </span>
                    ))}
                ...
                <div key={cat.id} className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs font-mono text-zinc-300 border-b border-zinc-800 pb-1"
                      >
                        {s}
                      </span>
                    ))}
