  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-300 font-mono selection:bg-zinc-200 selection:text-black">
      ...
      <div className="border-b border-zinc-900 px-6 py-4 flex justify-between items-center text-xs text-zinc-500">
        <span>
          $ cat /home/
          {p.name ? p.name.toLowerCase().replace(/\s+/g, "_") : "user"}
          /portfolio.sh
        </span>
        ...
        <section className="space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-zinc-400 font-semibold">
              {p.title || "Computer Science & Software Developer"}
            </p>
          </div>
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-4 bg-zinc-950 border border-zinc-900 rounded-lg space-y-2"
                >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="text-xs bg-zinc-900 border border-zinc-800 text-zinc-300 px-2 py-0.5 rounded"
                      >
                        {sk}
                      </span>
                    ))}
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-4">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-5 bg-zinc-950 border border-zinc-900 rounded-lg space-y-2"
                >
