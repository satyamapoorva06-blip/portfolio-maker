          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-zinc-800 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-pink-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Layout className="w-4 h-4 text-pink-400" /> FIGMA & UI/UX
                SYSTEM ARCHITECT
              </span>
              <h1 className="text-4xl sm:text-5xl font-black text-white">
                {p.name}
              </h1>
              <p className="text-base text-pink-300 font-mono">
                {p.title || "UI/UX & Web Designer"}
              </p>
            </div>
            <div className="flex gap-3">
              {p.socials?.github && (
                <a
                  ...
                  className="p-2.5 bg-slate-900 border border-slate-800 rounded-xl text-slate-300 hover:text-pink-400 transition"
                >
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-zinc-800 pb-3">
              <Sparkles className="w-5 h-5 text-pink-400" /> Figma & Interface
              Stack
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#111116] border border-zinc-800 rounded-2xl space-y-4"
                >
                  <h3 className="text-sm font-bold text-pink-300 font-mono uppercase">
                    {grp.category}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {grp.skills.map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-pink-950 text-pink-200 text-xs font-mono rounded-lg border border-pink-800"
                      >
                        {sk}
                      </span>
                    ))}
