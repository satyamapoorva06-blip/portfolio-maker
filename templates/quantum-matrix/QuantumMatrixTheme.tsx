  ...
  return (
    <div className="min-h-screen bg-[#040810] text-cyan-100 font-sans selection:bg-cyan-500 selection:text-black">
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-cyan-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-cyan-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Activity className="w-4 h-4 text-cyan-400" /> QUANTUM AI &
                FULLSTACK ARCHITECT
              </span>
              ...
              <p className="text-base text-cyan-300 font-mono">
                {p.title || "Quantum FullStack Developer"}
              </p>
            </div>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div
                  key={grp.id}
                  className="p-6 bg-[#081224] border border-cyan-900/50 rounded-2xl space-y-4"
                >
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.projects.map((proj) => (
                <div
                  key={proj.id}
                  className="p-6 bg-[#081224] border border-cyan-900/50 rounded-2xl space-y-4 hover:border-cyan-400 transition"
                >
      ...
      <footer className="border-t border-cyan-950 py-8 text-center text-xs text-slate-500 font-mono">
        © {new Date().getFullYear()} {p.name}. Quantum Fullstack Matrix
        Portfolio.
      </footer>
    </div>
  );
