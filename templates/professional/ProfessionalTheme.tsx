          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
            ...
            <div className="flex flex-col gap-2.5 text-xs text-slate-300">
              <a
                ...
                className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold rounded-xl shadow-md flex items-center gap-2 transition"
              >
              ...
              {personal.socials.github && (
                <a
                  ...
                  className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-cyan-300 rounded-xl border border-slate-800 flex items-center gap-2 transition"
                >
        ...
        <div className="p-8 sm:p-12 space-y-12">
          ...
          {about.summary && (
            <section className="space-y-4">
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200 pb-2">
                Executive Summary
              </h2>
            ...
            <section className="space-y-6">
              <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Code className="w-4 h-4 text-slate-800" /> Key Projects &
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {projects.map((proj) => (
                  <div
                    key={proj.id}
                    className="p-6 bg-slate-50 rounded-2xl border border-slate-200 space-y-4 flex flex-col justify-between hover:border-slate-400 transition"
                  >
                    <div className="space-y-2">
                      <div className="flex justify-between items-start">
                        ...
                        {proj.githubUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="p-2 bg-white rounded-lg border border-slate-200 text-slate-700 hover:text-cyan-600 transition"
                          >
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-2 border-t border-slate-200">
                      {proj.technologies.map((tech, idx) => (
                        <span
                          key={idx}
                          className="text-[11px] font-mono bg-white text-slate-700 px-2.5 py-0.5 rounded border border-slate-200"
                        >
                          {tech}
                        </span>
                      ))}
          ...
          {sectionVisibility.experience && experience.length > 0 && (
            <section className="space-y-6">
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200 pb-2 flex items-center gap-2">
                <Briefcase className="w-4 h-4 text-slate-800" /> Professional
                Experience
              </h2>
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {cat.skills.map((s, idx) => (
                        <span
                          key={idx}
                          className="text-xs bg-white text-slate-800 font-mono px-2.5 py-1 rounded border border-slate-200"
                        >
                          {s}
                        </span>
                      ))}
          ...
          {sectionVisibility.education && education.length > 0 && (
            <section className="space-y-6">
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200 pb-2 flex items-center gap-2">
                <GraduationCap className="w-4 h-4 text-slate-800" /> Education &
                Qualifications
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {education.map((edu) => (
