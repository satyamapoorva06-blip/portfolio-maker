...
export default function AcademicScholarTheme({
  data,
}: {
  data: PortfolioData;
}) {
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-indigo-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-indigo-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <BookOpen className="w-4 h-4 text-indigo-400" /> ACADEMIC AI
                SCHOLAR & RESEARCHER
              </span>
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              {data.education.map((edu) => (
                <div
                  key={edu.id}
                  className="p-6 bg-[#0b162a] border border-indigo-900/50 rounded-2xl flex flex-wrap justify-between items-center gap-4"
                >
                  <div>
