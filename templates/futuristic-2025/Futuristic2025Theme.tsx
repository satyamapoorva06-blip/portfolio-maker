        ...
        {projects && projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Eye className="w-5 h-5 text-cyan-400" /> Holographic Showcase &
              Systems
            </h2>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {proj.technologies.map((t, i) => (
                      <span
                        key={i}
                        className="text-[10px] bg-cyan-950/90 text-cyan-300 px-2.5 py-0.5 rounded-lg border border-cyan-800 font-mono"
                      >
                        {t}
                      </span>
                    ))}
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((cat) => (
                <div
                  key={cat.id}
                  className="p-6 bg-[#0b0d18]/70 border border-cyan-500/30 rounded-3xl space-y-3 backdrop-blur-xl"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.skills.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-[#121526] text-slate-200 px-3 py-1 rounded-xl border border-cyan-500/20 font-mono"
                      >
                        {s}
                      </span>
                    ))}
