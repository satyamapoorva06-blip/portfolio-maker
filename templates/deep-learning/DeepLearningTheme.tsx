...
import { Cpu, Brain, Network, Github, Linkedin, ExternalLink } from 'lucide-react';

export default function DeepLearningTheme({ data }: { data: PortfolioData }) {
  const p = data.personal;
  const a = data.about;

  return (
    <div className="min-h-screen bg-[#04060b] text-indigo-100 font-sans selection:bg-purple-500 selection:text-white">
      <header className="border-b border-indigo-900/50 bg-[#070b14]/90 backdrop-blur sticky top-0 z-50 px-6 py-4">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 font-mono text-sm text-indigo-400">
            <Brain className="w-5 h-5 text-indigo-400" />
            <span className="font-bold text-white tracking-wider">{p.name || 'Deep Learning Engineer'}</span>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-12">
        <section className="p-8 bg-[#090e1b] border border-indigo-500/30 rounded-3xl space-y-6 shadow-2xl">
          <div className="flex flex-wrap justify-between items-start gap-4 border-b border-indigo-900/50 pb-6">
            <div className="space-y-2">
              <span className="text-xs font-mono text-indigo-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                <Network className="w-4 h-4 text-indigo-400" /> DEEP NEURAL NETWORK SPECIALIST
              </span>
              <h1 className="text-4xl sm:text-5xl font-black text-white">{p.name}</h1>
              <p className="text-base text-indigo-300 font-mono">{p.title || 'Deep Learning & Neural Network Engineer'}</p>
            </div>
        ...
        {data.skills && data.skills.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-indigo-950 pb-3">
              <Cpu className="w-5 h-5 text-indigo-400" /> Deep Learning Frameworks & Architectures
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.skills.map((grp) => (
                <div key={grp.id} className="p-6 bg-[#090e1b] border border-indigo-900/50 rounded-2xl space-y-4">
                  <h3 className="text-sm font-bold text-indigo-300 font-mono uppercase">{grp.category}</h3>
                  <div className="flex flex-wrap gap-2">
                    {grp.skills.map((sk, idx) => (
                      <span key={idx} className="px-3 py-1 bg-indigo-950 text-indigo-200 text-xs font-mono rounded-lg border border-indigo-800">
                        {sk}
                      </span>
                    ))}
                  </div>
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-extrabold text-white flex items-center gap-2 border-b border-indigo-950 pb-3">
              <Network className="w-5 h-5 text-indigo-400" /> Deep Learning Models & Research
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.projects.map((proj) => (
                <div key={proj.id} className="p-6 bg-[#090e1b] border border-indigo-900/50 rounded-2xl space-y-4 hover:border-indigo-400 transition">
      ...
      <footer className="border-t border-indigo-950 py-8 text-center text-xs text-slate-500 font-mono">
        © {new Date().getFullYear()} {p.name}. Deep Learning Neural Portfolio.
      </footer>
    </div>
  );
}
